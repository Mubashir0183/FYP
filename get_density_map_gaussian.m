function im_density = get_density_map_gaussian(im,annPoints)


im_density = zeros(size(im)); 
[h,w] = size(im_density);

if(length(annPoints)==0)
    return;
end

if(length(annPoints(:,1))==1)
    x1 = max(1,min(w,round(annPoints(1,1))));
    y1 = max(1,min(h,round(annPoints(1,2))));
    im_density(y1,x1) = 255;
    return;
end
for j = 1:length(annPoints) 	
    f_sz = 15;
    sigma = 4.0;
    H = fspecial('Gaussian',[f_sz, f_sz],sigma);
    x = min(w,max(1,abs(int32(floor(annPoints(j,1)))))); 
    y = min(h,max(1,abs(int32(floor(annPoints(j,2))))));
    if(x > w || y > h)
        continue;
    end
